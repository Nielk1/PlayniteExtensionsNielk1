...
$extensionPaths = @{
    'RobotCache' = 'source\Libraries\RobotCacheLibrary\'
...
$langSourceDir = Join-Path $repoPath "PlayniteExtensionsRepo\source\Localization\"
$custLangSourceDir = Join-Path $repoPath "source\Localization\"
...
foreach ($extension in $extensionPaths.Keys)
{
    foreach ($langFile in (Get-ChildItem -LiteralPath $langSourceDir -Filter "*.xaml"))
    {
        ...
        [xml]$langXmlCust = Get-Content -LiteralPath (Join-Path $custLangSourceDir $langFile.Name) -Raw -ErrorAction Ignore
        $parsingExt = $null
        foreach ($node in $langXmlCust.ResourceDictionary.ChildNodes)
        {
            if ($node.Name -eq "#comment")
            {
                $parsingExt = $node.InnerText.Trim()
            ...
            elseif ($parsingExt -eq $extension -or ($extension -eq 'UniversalSteamMetadata' -and $parsingExt -eq "Steam"))
            {
                ...
                $overwriteNode = $targetXml.ResourceDictionary.String | Where-Object { $_.Key -eq $nodeClone.Key }
                if ($overwriteNode -ne $null){
                    $overwriteNode.ParentNode.RemoveChild($overwriteNode)
                }
